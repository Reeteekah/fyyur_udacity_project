...
class Shows(db.Model):
    ...
    start_time = db.Column(db.String(120), default=datetime.now(), nullable=False)
...
def show_venue(venue_id):
    ...
    data = {
        ...
        'city': venue.city,
        'state': venue.state,
        'genres': re.split(',|{|}', venue.genres)[1:-1],
        'phone': venue.phone,
    ...
    past = db.session.query(Shows).filter(Shows.venue_id == venue_id)\
        .filter(Shows.start_time < datetime.now()).join(Artist, Shows.artist_id == Artist.id)\
...
def show_artist(artist_id):
    ...
    data = {
        ...
        'city': artist.city,
        'state': artist.state,
        'genres': re.split(',|{|}', artist.genres)[1:-1],
        'phone': artist.phone,
    ...
    past = db.session.query(Shows).filter(Shows.artist_id == artist_id)\
        .filter(Shows.start_time < datetime.now()).join(Venue, Shows.venue_id == Venue.id)\
